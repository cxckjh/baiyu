define(['jquery','myutil','app/myfn'],function ($,x,url){
	 function getNav5Data(root3) {
        var xhr = x();  //创建ajax对象
        xhr.open('get',url.getBaseURL() + '/zcityWalk');
        xhr.send(null);
        xhr.onreadystatechange = function (e) {
            if(xhr.readyState === 4){
              var navs8 = JSON.parse(xhr.responseText);
                console.log(navs8)
                navs8.forEach(function (index,elem) {
                   var li=$('<li class="zhutili"></li>').appendTo(root3)
